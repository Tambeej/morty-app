 ...
 * Tests the showSuccess/showError/showInfo convenience methods
 * as well as addToast/removeToast low-level API.
 */
...
describe('ToastContext', () => {
  ...
  it('should not auto-dismiss when duration is 0', async () => {
    renderWithToast();
    await act(async () => {
      await userEvent.click(screen.getByText('Show Info (no dismiss)'));
    });
    expect(screen.getByTestId('toast-count').textContent).toBe('1');
